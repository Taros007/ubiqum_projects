...
set.seed(541)
# train and test
train_ids <- createDataPartition(y = existingDummy$Product_type.Laptop,
                                 p = 0.75,
                                 list = F)
...
Filtered <- filter(test, Product_type.Laptop == 1)
cat("Amount of laptops in testset is:", nrow(Filtered))
