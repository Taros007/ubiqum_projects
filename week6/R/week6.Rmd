...
In addition to analysing on a product level, it is also useful to look at a product type level. The chart below shows that the target company is strong in the Desktop, and Laptop divisions. These are both categories that are weak performers within our company. If compared to the Blackwell product types that do well (see chart below), it is really interesting to see that our bestseller, game consoles, are not even being offered by Electronidex.
...
blackwell %>% 
  ...
  labs(title = "Blackwell: sales volume per product category") +
...
Next, we will use a market basket analysis in order to gain insight in which products are generally sold together. This will hopefully provide some clues as to which products can be recommended together, and whether there is potential for upside.
...
inspect(head(sort(rules1, by = "count"),10))
#plot(rules1, method = "graph", shading = NA)
```
