...
cateTrans <- read.transactions('../input/replaced.csv', sep = ',', rm.duplicates = F, format = "basket")
...
#Clean up variables
remove(activeheadphones, cables, computers, desktops, harddrives, headphones, iodevices, laptops, monitors, other, printer, speakers, stands, tablets, smarthome, i)
...
The first dataset to explore is the data as provided. Requiring a minimum length of 2 products to avoid single item rules, as well as a confidence of at least 50% and a very low support of 1%. 

```{r Building model on original data, echo = F, results = "hide", message = FALSE, warning = FALSE}
## Building of the model ====================================

rules <- apriori(elecTransactions, 
                 parameter = list(support = 0.01, confidence = 0.2, minlen = 1, maxlen = 2)
                 )

head(inspect(sort(rules, by = "lift")), 5)
#arulesViz::ruleExplorer(rules)
#plot(rules, method = "graph", shading = NA)
```

However, given that both laptops and desktops can be considered the main products in a purchase, these are removed from the rhs side of rules.
