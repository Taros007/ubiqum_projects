...
<i>Type of clients</i>
From an initial scan of the data, it soon became pretty clear that the client group is not homogenous. One would except transactions that include one type of desktop or laptop, and some complementary items (such as a keyboard or printer). In addition, there are transactions for only complementary items. However, a large amount of transactions include multiple desktops and laptops, indicating that the buyer might be a organisation instead of a private individual.

To split the data, separate datasets have been created using a corporate flag. If more than one type of desktop or laptop was purchased, the transaction was flagged as corporate.
...
division <- tribble(~whatever, ~buyertype, ~amount,
                    "whatever", "corporate", length(corporation[corporation == TRUE]),
                    "whatever",  "private individual", length(corporation[corporation == FALSE]))

division %>%
  ggplot(aes(x = whatever, y = amount, fill = buyertype)) +
...
There are two levels on which the data can be analysed: product and producttype. The latter data has been 
...
#Start of with functions for desktops and laptops, as this is used later on for analysis of corporates and private individuals as well
...
##Create data with only laptops and desktops transformed
...
#Clean up variables
remove(activeheadphones, cables, computers, desktops, harddrives, headphones, iodevices, laptops, monitors, other, printer, speakers, stands, tablets, smarthome, i, cateTrans)
...
blackwell %>% 
  ...
  labs(title = "Sales volume per product category") +
  geom_label(aes(x = ProductType, y = n, label = round(n, 0)),
             hjust = 1, 
             vjust = 0.5, 
             colour = "white", 
             fill = NA, 
...
Next, we assess which categories of products are usually bought together. As we know that desktop and laptops are usually the main item in a purchase, we make sure that these are only found in the left hand side of the rule equation.


[TO DO
Create rules for categories
Investigate which products are not selling much, and with which products they are sold -> promotions
Dig into categories for region 4 (online only) for Blackwell
]


```{r Building model with categories, echo = F, results = "hide", message = FALSE, warning = FALSE}
...
```{r Building model with desktop and laptops, echo = F, results = "hide", message = FALSE, warning = FALSE}
## Digging into Laptops =====================================
rules1 <- apriori(elecDeskLap, 
                 parameter = list(support = 0.01, confidence = 0.1, minlen = 2),
                 appearance = list(default="rhs", lhs=c("Laptop", "Desktop"))
                  )
inspect(sort(rules1, by = "count"))
#plot(rules1, method = "graph", shading = NA)
```

```{r Building model corporates, echo = F, results = "hide", message = FALSE, warning = FALSE}

elecCorp <- elecTransactions[corporation == T]
...
```{r Building model private individuals, echo = F, results = "hide", message = FALSE, warning = FALSE}

elecCorp <- elecTransactions[corporation == F]
