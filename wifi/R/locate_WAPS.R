...
for (wap in WAPlocations$WAP) {
  selected_wap <- wifiData %>% select(wap, LATITUDE, LONGITUDE) %>% filter_all(all_vars(. != -200))
  if (nrow(selected_wap) != 0) {
    total_rssi <- sum(selected_wap %>% select(!!wap))
    WAPlocations[i,2] <- sum(selected_wap %>% select(wap) * selected_wap$LATITUDE) / total_rssi
    WAPlocations[i,3] <- sum(selected_wap %>% select(wap) * selected_wap$LONGITUDE) / total_rssi
  }
  i <- i + 1
}
